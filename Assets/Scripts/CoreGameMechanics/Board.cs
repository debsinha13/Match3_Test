...
public class Board : MonoBehaviour
{
    ...
    [SerializeField] private FXSymbolsPool _fxSymbolsPool;
    //public GameObject[] gamePiecePrefabs;

    [SerializeField] private Cascading[] _symbolCascading;
    [SerializeField] private SO_AnimationAttributes _swapAttributes;
    ...
    private WaitForSeconds _waitSwitchTiles;
    [SerializeField] private float delayForMatchHighlight = 0.2f;
    WaitForSeconds _waitForHighlight;
    void Start()
    {
        ...
        _waitSwitchTiles = new WaitForSeconds(_swapAttributes.Time());
        _waitForHighlight = new WaitForSeconds(delayForMatchHighlight);
    }
    ...
    IEnumerator SwitchTilesRoutine(Tile clickedTile, Tile targetTile)
    {
        if (m_playerInputEnabled)
        {
            ...
            {
                clickedPiece.MoveOnSwap(targetTile.XIndex, targetTile.YIndex);
                targetPiece.MoveOnSwap(clickedTile.XIndex, clickedTile.YIndex);
                
                yield return _waitSwitchTiles;
                AssignPieceToBoard(clickedPiece, targetTile.XIndex, targetTile.YIndex);
                AssignPieceToBoard(targetPiece, clickedTile.XIndex, clickedTile.YIndex);
                ...
                {
                    clickedPiece.MoveOnSwap(clickedTile.XIndex, clickedTile.YIndex);
                    targetPiece.MoveOnSwap(targetTile.XIndex, targetTile.YIndex);
                    AssignPieceToBoard(clickedPiece, clickedTile.XIndex, clickedTile.YIndex);
                    AssignPieceToBoard(targetPiece, targetTile.XIndex, targetTile.YIndex);
                }
                else
                {
                    yield return _waitSwitchTiles;
            ...
            if (piece != null)
            {
                ...
                switch (piece.type)
                {
                    case MatchType.Broom:
                        s = _fxSymbolsPool._broomPrefabFxPool.Get();
                        s.Init(_fxSymbolsPool._broomPrefabFxPool, piece.transform.position);
                        break;
                    case MatchType.Shield:
                        s = _fxSymbolsPool._shieldPrefabFxPool.Get();
                        s.Init(_fxSymbolsPool._shieldPrefabFxPool, piece.transform.position);
                        break;
                    case MatchType.Cracker:
                        s = _fxSymbolsPool._crackerPrefabFxPool.Get();
                        s.Init(_fxSymbolsPool._crackerPrefabFxPool, piece.transform.position);
                        break;
                    case MatchType.Square:
                        s = _fxSymbolsPool._squarePrefabFxPool.Get();
                        s.Init(_fxSymbolsPool._squarePrefabFxPool, piece.transform.position);
                        break;
                    case MatchType.Heart:
                        s = _fxSymbolsPool._heartPrefabFxPool.Get();
                        s.Init(_fxSymbolsPool._heartPrefabFxPool, piece.transform.position);
                        break;
                }
            }
        }
                ...
                for (int j = i + 1; j < height; j++)
                {
                    if (_allGamePieces[column, j] != null)
                    {
                        if (_symbolCascading[column].isCascading)
                        {
                            ...
                            _allGamePieces[column, j].MoveOnCollapseColumn(column, i, _symbolCascading[column].symbolCascadeCount);
                        }
                        else 
                        {
                            _allGamePieces[column, j].MoveOnCollapseColumn(column, i, 0);
            ...
            for (int j = 0; j < height; j++)
            {
                if (_allGamePieces[i, j] == null)
                {
                    ...
                    if (_symbolCascading[i].isCascading)
                    {
                        ...
                        piece.MovePieceOnRefill(_symbolSpawnPosition[i].transform.position, i, j, _symbolCascading[i].symbolCascadeCount);  
                    }
                    else
                    {
                        piece.MovePieceOnRefill(_symbolSpawnPosition[i].transform.position, i, j, _symbolCascading[i].symbolCascadeCount);;
